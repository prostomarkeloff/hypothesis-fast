"""Width-aware float helpers, ported from hypothesis.internal.floats.

Struct-based reinterpretation for widths 16/32/64. The hot, width-64 lexical
encoding (`float_to_lex`/`lex_to_float`/`is_simple`) lives in Rust
(`hypothesis_fast._engine`) and is re-exported from
`hypothesis_fast.internal.conjecture.floats`; this module stays pure Python
because it is only used for counting/clamping/bound helpers, not per-draw.
"""
...
def make_float_clamper(
    ...
    *,
    ...
    """Return a function that clamps floats into the given bounds."""
    from hypothesis_fast.internal.conjecture.choice import choice_permitted
...
def clamp(lower: float | int, value: float | int, upper: float | int) -> float | int:
    """Clamp `value` so that lower <= value <= upper.  NaN is mapped to lower."""
...
SIGNALING_NAN = int_to_float(0x7FF8_0000_0000_0001)
