"""Shape / broadcasting / index helpers for the array extras.

Native port of ``hypothesis.extra._array_helpers``: identical logic, but built on
the native ``hypothesis_fast`` strategies and a native ``defines_strategy`` so the
custom strategies draw against the native ConjectureData. Pure helpers
(``check_function``/``check_type``/``check_valid_interval``/``_calc_p_continue``/
``not_set``) are reused from real hypothesis unchanged.
"""
...
from hypothesis_fast import assume, strategies as st
from hypothesis_fast.errors import InvalidArgument
from hypothesis_fast.extra._lazy import defines_strategy

if TYPE_CHECKING:
    # Type-checking only: the real (generic, stubbed) SearchStrategy so annotations resolve.
    # At runtime SearchStrategy IS our native class (the compiled _engine ships no stubs).
...
# with minor upgrades to handle numeric and optional dimensions.
...
def _hypothesis_parse_gufunc_signature(signature):
    ...
    # no way to share an un-named dimension between shapes.
...
def mutually_broadcastable_shapes(
    ...
    * ``max_side`` is the largest size that an unaligned dimension can possess,
    ...
    ``input_shapes`` (a tuple of the N generated shapes) and ``result_shape``.
    """
...
class MutuallyBroadcastableShapesStrategy(SearchStrategy):
    def __init__(
        ...
        super().__init__()
        # Callers (broadcastable_shapes / mutually_broadcastable_shapes) always resolve
        # max_dims/max_side to concrete ints before constructing; the None defaults are only
        # API convenience. Narrow them so the dimension arithmetic below is well-typed.
        assert max_dims is not None and max_side is not None
    ...
    def _draw_core_dimensions(self, data):
        ...
        # that will not be present in the final shape. Only reached when a gufunc
        # signature is present (do_draw guards on self.signature is None).
    ...
    def _draw_loop_dimensions(self, data, use=None):
        ...
        for dim_count in range(1, self.max_dims + 1):
            ...
            # We begin by drawing a valid dimension-size for the given dimension.
            ...
            for shape_id, shape in enumerate(shapes):
                # Populating this dimension-size for each shape, either the drawn
                # size is used or, if permitted, a singleton dimension.
                ...
                # Use a trick where a biased coin is queried to see if the given
                # shape-tuple will continue to be grown.
